...
export default function CardCriar() {
  ...
  useEffect(() => {
    ...
    if (storedToken) {
      ...
      // Se não houver token, redirecionar para login ou mostrar erro
      router.push('/login'); // ou qualquer rota de login que você tenha
    }
  }, []);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!token) {
      setMensagem('Você precisa estar autenticado para cadastrar um aluno.');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({ name: '', turma: '', RA: '', senha: '' });
      } else {
        const errorData = await response.json();
        setMensagem(`Erro ao cadastrar aluno: ${errorData.message || 'Erro desconhecido'}`);
      }
    } catch (error) {
